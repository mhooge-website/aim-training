...
class CanvasHelper {
    ...
    static fillString(s, x, y) {
        this.drawCtx.fillText(s, x, y);
    }
    static fillCircle(x, y, d) {
        this.circlePath(x, y, d);
        this.drawCtx.fill();
    }
    static drawCircle(x, y, d) {
        this.circlePath(x, y, d);
        this.drawCtx.stroke();
    }
    static circlePath(x, y, d) {
    ...
    static drawRectangle(x, y, w, h) {
        this.drawCtx.strokeRect(x, y, w, h);
    }
    static fillRectangle(x, y, w, h) {
    ...
    static setHelperContext(ctx) {
        this.drawCtx = ctx;
    }
    static getRelativePos(xPos, yPos, canvas) {
...
CanvasHelper.BG_COLOR = "rgb(255, 255, 255)";
CanvasHelper.FG_COLOR = "rgb(0, 0, 0)";
exports.CanvasHelper = CanvasHelper;
},{}],3:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const canvashelper_1 = require("./canvashelper");
function drawNumber(n, x, y) {
    let counter = 0;
    let oldFontSize = parseInt(canvashelper_1.CanvasHelper.drawCtx.font.slice(0, 2));
    let maxFontSize = oldFontSize * 2;
    let fontSize = maxFontSize;
    let id = setInterval(() => {
        fontSize = fontSize - ((maxFontSize - oldFontSize) / 25);
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
        canvashelper_1.CanvasHelper.fillRectangle(x - 5, y - 100, 85, 105);
        canvashelper_1.CanvasHelper.setFont(fontSize + "px serif");
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.FG_COLOR);
        canvashelper_1.CanvasHelper.fillString(n + "!", x, y);
        counter++;
        if (counter > 25) {
            canvashelper_1.CanvasHelper.setFont(oldFontSize + "px serif");
            clearInterval(id);
        }
    }, 20);
}
function countdown(count, x, y) {
    setTimeout(() => {
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
        canvashelper_1.CanvasHelper.fillRectangle(x - 5, y - 100, 85, 105);
        drawNumber(count, x, y);
        if (count > 1)
            countdown(count - 1, x, y);
    }, 1000);
}
var Countdown;
(function (Countdown) {
    function createCountdown(x, y) {
        countdown(3, x, y);
    }
    Countdown.createCountdown = createCountdown;
})(Countdown = exports.Countdown || (exports.Countdown = {}));
},{"./canvashelper":2}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const game_1 = require("./game");
const moving_target_1 = require("./moving_target");
var MovePattern;
(function (MovePattern) {
    MovePattern[MovePattern["Random"] = 0] = "Random";
    MovePattern[MovePattern["Short"] = 1] = "Short";
    MovePattern[MovePattern["Long"] = 2] = "Long";
})(MovePattern || (MovePattern = {}));
class GameCustom extends game_1.Game {
    constructor() {
        super(...arguments);
        this.targetMovePattern = null;
        this.targetMoveSpeed = 1;
        this.targetMoveY = false;
        this.targetLifetime = 0;
        this.targetGrows = false;
        this.maxActiveTargets = 1;
        this.spawnDelay = 1000;
        this.targetColor = "rgb(255, 0, 0)";
    ...
    init() {
        ...
        if (this.targetGrows || this.targetMovePattern != null) {
            if (this.targetLifetime > 0)
                this.animationLoop(this.targetLifetime);
            else
                this.animationLoop(2000);
        }
    }
    animationLoop(duration) {
        let frames = 40;
        let deltaSize = this.targetDiameter / (frames / 2);
        setInterval(() => {
            if (this.targetGrows)
                super.resizeTargets(duration, deltaSize);
            if (this.targetMovePattern != null)
                this.moveTargets();
        }, duration / frames);
    }
    move(dir, changeDir, target, xAxis) {
        ...
        if (xAxis) {
            target.x += delta * dir;
            if (target.x > this.canvas.width - targetRadius)
                target.x = this.canvas.width - targetRadius;
            else if (target.x < targetRadius)
                target.x = targetRadius;
        }
        else {
            target.y += delta * dir;
            if (target.y > this.canvas.height - targetRadius)
                target.y = this.canvas.height - targetRadius;
    ...
    setMovePattern() {
        let moveChckBox = $("#custom-moving-targets").get(0);
        let targetsMove = moveChckBox.checked;
        if (targetsMove) {
            let moveRandomChckBox = $("#custom-move-random").get(0);
            if (moveRandomChckBox.checked) {
                this.targetMovePattern = MovePattern.Random;
                return;
            }
            let moveShortChckBox = $("#custom-move-short").get(0);
    ...
    setMoveSpeed() {
        let speedInput = $("#custom-move-speed").get(0);
        this.targetMoveSpeed = this.clampInputValue(speedInput);
    }
    setMoveY() {
        this.targetMoveY = $("#custom-move-y").get(0).checked;
    }
    setTargetLifetime() {
        let lifetimeInput = $("#custom-target-lifetime").get(0);
        this.targetLifetime = this.clampInputValue(lifetimeInput) * 1000;
    }
    setTargetGrows() {
        this.targetGrows = $("#custom-target-grows").get(0).checked;
    }
    setTargetDiameter() {
        let diameterInput = $("#custom-target-diameter").get(0);
        this.targetDiameter = this.clampInputValue(diameterInput);
    }
    setMaxActiveTargets() {
        let activeTargetsInput = $("#custom-max-targets").get(0);
        this.maxActiveTargets = this.clampInputValue(activeTargetsInput);
    }
    setSpawnDelay() {
        let spawnDelayInput = $("#custom-spawn-delay").get(0);
        this.spawnDelay = this.clampInputValue(spawnDelayInput) * 1000;
    }
    setTargetColor() {
        this.targetColor = $("#custom-target-color").get(0).value;
    }
    setGameParameters() {
        this.setMovePattern();
        this.setMoveSpeed();
    ...
    gameLoop() {
        setTimeout(() => {
            ...
            let target = this.targetMovePattern != null ? new moving_target_1.MovingTarget(x, y, d, this.targetColor) : super.createTarget(x, y, d, this.targetColor);
...
exports.GameCustom = GameCustom;
},{"./game":5,"./moving_target":10}],5:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
class Game {
    ...
    createTarget(x, y, d, color = "rgb(255, 0, 0)") {
        let target = new target_1.Target(x, y, d, color);
        return target;
    }
    startTargetLifetimeCounter(target, lifetime) {
    ...
    addCanvasListener() {
        this.canvas.addEventListener("click", (e) => {
            this.totalClicks += 1;
            let pos = canvashelper_1.CanvasHelper.getRelativePos(e.clientX, e.clientY, this.canvas);
            for (let i = 0; i < this.activeTargets.length; i++) {
    ...
    init() {
        ...
        let nightButton = $("#toggle-night-button").get(0);
        nightButton.onclick = (ev) => {
            this.toggleNightMode(nightButton);
        };
        canvashelper_1.CanvasHelper.setHelperContext(this.canvas.getContext("2d"));
        if (this.nightMode)
            this.enableNightMode();
        else
            this.disableNightMode();
        canvashelper_1.CanvasHelper.setFont("24px serif");
        this.addCanvasListener();
        this.drawMetrics();
    }
    drawTarget(target) {
        canvashelper_1.CanvasHelper.setFillColor(target.color);
        canvashelper_1.CanvasHelper.fillCircle(target.x, target.y, target.d / 2);
    }
    eraseTarget(target) {
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
        canvashelper_1.CanvasHelper.fillCircle(target.x - 1, target.y - 1, (target.d / 2) + 2);
    }
    getRandomX(borderOffset = 0) {
        let x = Math.random() * this.canvas.width;
        if (x + borderOffset > this.canvas.width)
            x = this.canvas.width - borderOffset;
        return x;
    }
    getRandomY(borderOffset = 0) {
        let y = Math.random() * this.canvas.height;
        if (y + borderOffset > this.canvas.height)
            y = this.canvas.height - borderOffset;
        return y;
    }
    enableNightMode(btn = null) {
        let bgColor = "rgb(30, 30, 30)";
        let fgColor = "rgb(255, 255, 255)";
        document.body.style.backgroundColor = bgColor;
        canvashelper_1.CanvasHelper.setBackgroundColor(bgColor);
        canvashelper_1.CanvasHelper.setForegroundColor(fgColor);
        $("#metrics-div").get(0).style.color = fgColor;
        if (btn != null) {
            let img = btn.firstChild;
            img.src = "/aim-training/resources/light_on.png";
        }
        this.nightMode = true;
    }
    disableNightMode(btn = null) {
    ...
    drawPointsForHit(target, points) {
        let x = target.x;
        let y = target.y - 10;
        canvashelper_1.CanvasHelper.setFont("16px serif");
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.FG_COLOR);
        canvashelper_1.CanvasHelper.fillString("+" + points.toFixed(0), x, y);
        canvashelper_1.CanvasHelper.fillString(this.reactTimes[this.reactTimes.length - 1] + " ms", x - 10, y + 15);
        setTimeout(() => {
            canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
            canvashelper_1.CanvasHelper.fillRectangle(x - 15, y - 30, 60, 55);
        }, 1000);
    }
    resizeTargets(lifetime, delta) {
        for (let i = 0; i < this.activeTargets.length; i++) {
            let target = this.activeTargets[i];
            if (target == undefined) {
                continue;
    ...
    drawMetrics() {
        let scoreLabel = $("#metric-score").get(0);
        let accuracyLabel = $("#metric-accuracy").get(0);
        let scoreReact = $("#metric-react-time").get(0);
    ...
    redraw() {
        canvashelper_1.CanvasHelper.eraseAll();
...
exports.Game = Game;
},{"./canvashelper":2,"./target":11,"jquery":1}],6:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const game_1 = require("./game");
const canvashelper_1 = require("./canvashelper");
const countdown_1 = require("./countdown");
class GameHoldAngle extends game_1.Game {
    constructor(name) {
        super(name);
        this.wallWidth = 20;
        this.maxPeekLength = 200;
        this.peekSpeedMS = 200;
        this.intervalId = 0;
    }
    init() {
        super.init();
    ...
    drawWallMarkers(x, yTop, yBot) {
        canvashelper_1.CanvasHelper.setStrokeColor("rgb(255, 0, 0)");
        let xShift = x > this.canvas.width / 2 ? 40 : -40;
        canvashelper_1.CanvasHelper.drawLine(x + xShift, yTop, x, yTop);
        canvashelper_1.CanvasHelper.drawLine(x + xShift, yBot, x, yBot);
    }
    drawWall(x, scale) {
        canvashelper_1.CanvasHelper.setFillColor("rgb(0, 0, 0)");
        canvashelper_1.CanvasHelper.fillRectangle(x, 0, this.wallWidth * scale, this.canvas.height);
    }
    getPeekLength() {
        return Math.random() * this.maxPeekLength + (this.targetDiameter / 2);
    }
    animatePeek(x, target, scale) {
    ...
    eraseInfrontOfWall(x) {
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
        if (x > this.canvas.width / 2) {
            canvashelper_1.CanvasHelper.fillRectangle(0, 0, x, this.canvas.height);
        }
        else if (x < this.canvas.width / 2) {
            canvashelper_1.CanvasHelper.fillRectangle(x, 0, this.canvas.width, this.canvas.height);
        }
    }
    eraseBehindWall(x) {
        canvashelper_1.CanvasHelper.setFillColor(canvashelper_1.CanvasHelper.BG_COLOR);
    ...
    redrawObjects(x, yTop, yBot, scale) {
    ...
    gameLoop() {
        ...
        countdown_1.Countdown.createCountdown(x + this.wallWidth + 5, headPos.top - 10);
...
},{"./canvashelper":2,"./countdown":3,"./game":5}],7:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const game_1 = require("./game");
const canvashelper_1 = require("./canvashelper");
const countdown_1 = require("./countdown");
class GameFlick extends game_1.Game {
    targetHit(target) {
        super.targetHit(target);
        canvashelper_1.CanvasHelper.eraseAll();
        this.activeTargets = [];
    ...
    gameLoop() {
        ...
        let y = super.getRandomY(this.targetDiameter);
        canvashelper_1.CanvasHelper.setFont("50px serif");
        countdown_1.Countdown.createCountdown(this.canvas.offsetWidth / 2 + 10, this.canvas.height / 2 - 30);
        setTimeout(() => {
...
exports.GameFlick = GameFlick;
},{"./canvashelper":2,"./countdown":3,"./game":5}],8:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const game_1 = require("./game");
class GameSwarm extends game_1.Game {
    constructor() {
        super(...arguments);
...
exports.GameSwarm = GameSwarm;
},{"./game":5}],9:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const gamemode1_1 = require("./gamemode1");
const gamemode2_1 = require("./gamemode2");
const gamemode3_1 = require("./gamemode3");
const custom_gamemode_1 = require("./custom_gamemode");
const $ = require("jquery");
var currentGame;
...
function initializeGame(gamemode) {
    switch (gamemode) {
        case 0:
            ...
            break;
        case 1:
            currentGame = new gamemode2_1.GameFlick("Flick");
            break;
        case 2:
            currentGame = new gamemode3_1.GameSwarm("Swarm");
            break;
        case 3:
            currentGame = new custom_gamemode_1.GameCustom("Swarm");
            showCustomGameMenu();
            break;
    }
...
function setupCustomGameMenu() {
    let customChck = $("#custom-moving-targets").get(0);
    ...
    for (let i = 0; i < 3; i++) {
        let radioButton = customMoveRadios[i];
        let otherButtons = [];
        for (let j = 0; j < 3; j++) {
            if (i != j) {
                let otherButton = customMoveRadios[j];
...
},{"./custom_gamemode":4,"./gamemode1":6,"./gamemode2":7,"./gamemode3":8,"jquery":1}],10:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const target_1 = require("./target");
class MovingTarget extends target_1.Target {
    constructor(x, y, d, color = "rgb(255, 0, 0)") {
        super(x, y, d, color);
        this.startX = x;
        this.startY = y;
    }
...
},{"./target":11}],11:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
class Target {
    constructor(x, y, d, color = "rgb(255, 0, 0)") {
